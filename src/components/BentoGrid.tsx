...
const BentoGrid = ({ children, className }: { children: ReactNode; className?: string }) => {
  return <div className={clsxm('grid w-full auto-rows-[22rem] grid-cols-3 gap-4', className)}>{children}</div>;
};
...
const BentoCard = ({ name, className, background, Icon, description }: { name: string; className: string; background: ReactNode; Icon: any; description: string; href: string; cta: string }) => (
  ...
  >
    ...
    <div className={clsxm('pointer-events-none absolute bottom-0 flex w-full translate-y-10 transform-gpu flex-row items-center p-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100')}></div>
